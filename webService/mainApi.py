...
async def get_filtered_data(time_filter_request: TimeFilterRequest):
    try:
        time_filter = TimeFilter(
            time_filter_request.start_time, time_filter_request.end_time
        )
        cursor = collection.find({})
        data = [doc for doc in cursor]
        filtered_data = []
...
async def get_filtered_data_by_temperature(
    temperature_filter_request: TemperatureFilterRequest,
):
    try:
        cursor = collection.find({})
        data = [doc for doc in cursor]
        ...
        for item in data:
            ...
            if "temperature" in item:
                if (
                    temperature_filter_request.min_temperature is None
                    or item["temperature"] >= temperature_filter_request.min_temperature
                ) and (
                    temperature_filter_request.max_temperature is None
                    or item["temperature"] <= temperature_filter_request.max_temperature
                ):
                    filtered_data.append(item)
        return jsonable_encoder(filtered_data)
    except Exception as e:
...
async def get_filtered_data_by_humidity(humidity_filter_request: HumidityFilterRequest):
    try:
        ...
        for item in data:
            ...
            if "humidity" in item:
                if (
                    humidity_filter_request.min_humidity is None
                    or item["humidity"] >= humidity_filter_request.min_humidity
                ) and (
                    humidity_filter_request.max_humidity is None
                    or item["humidity"] <= humidity_filter_request.max_humidity
                ):
                    filtered_data.append(item)
        return jsonable_encoder(filtered_data)
    except Exception as e:
